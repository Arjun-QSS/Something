...
async function main() {
	...
	const nDaysPassed = () => randomBetweenRange(2, 5);
	...
	for (let i = 0; i < nDays; i++) {
		...
		for (let j = 0; j < classes.length; j++) {
			...
			const nLessons = randomBetweenRange(lessons.length - 2, lessons.length);

			for (let k = 0; k < nLessons; k++) {
				await prisma.lesson.create({
					data: {
						date,
						startTime: date.split("T")[0] + "T" + lessonTimes[k].split("T")[1],
						endTime:
							date.split("T")[0] + "T" + lessonTimes[k + 1].split("T")[1],
						...
						teacher: {
							connect: {
								id: lessons[k].teachers[
									Math.floor(Math.random() * lessons[k].teachers.length)
								].id,
							},
						},
					},
	...
	(async () => {
		await prisma.complaint.create({
			data: {
				...
				student: {
					connect: {
						id: (await prisma.student.findMany({ where: {}, take: 10 }))[0].id,
					},
				},
			},
		});
		await prisma.complaint.create({
			data: {
				...
				teacher: {
					connect: {
						id: (await prisma.teacher.findMany({ where: {}, take: 10 }))[0].id,
					},
				},
			},
		});
